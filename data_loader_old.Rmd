...
```{r}
shortdata = shortdata_backup

print(streamlit_formulas)

needs_eval <- streamlit_formulas %>%
  filter(type != 'x') %>%
  mutate(variable = gsub("\\(([^,]+)", "(shortdata$\\1", variable)) %>%
  select(variable) %>%
  unlist()

for (i in 1:length(needs_eval)) {
  new_var_name = needs_eval[i]
  if (new_var_name %!in% names(shortdata)) {
    shortdata[[new_var_name]] = NA
  }
}

remaining_vars <- needs_eval
max_iterations <- 10
iterations <- 0

for (i in 1:length(needs_eval)) {
  new_var <- needs_eval[i]
  expression <- gsub('`', '', needs_eval[i])
  print(expression)
  
  tryCatch({
    shortdata[[new_var]] <- eval(parse(text = expression))
    remaining_vars <- setdiff(remaining_vars, new_var)
  }, error = function(e) {
    print(e)
  })
}
...
if (length(remaining_vars) > 0) {
  warning("Some variables could not be computed.")
}
...
# modified for testing reason!!! REMOVE LATER

backpropagate_NAs <- function(df, default_value = 1, exclude_cols = c("year")) {
  # Identify numeric columns, excluding specified columns
  numeric_cols <- sapply(df, is.numeric)
  numeric_cols <- setdiff(names(which(numeric_cols)), exclude_cols)
  
  # Separate the last row
  last_row <- df[nrow(df), , drop = FALSE]
  df_without_last <- df[-nrow(df), ]
  
  for (col in numeric_cols) {
    # If the entire column (excluding last row) is NA, fill with default value
    if (all(is.na(df_without_last[[col]]))) {
      df_without_last[[col]] <- default_value
    } else {
      # Find last non-NA value for each NA
      last_known <- na.locf(df_without_last[[col]], fromLast = TRUE, na.rm = FALSE)
      
      # Replace NAs with last known value or default value
      df_without_last[[col]] <- ifelse(is.na(df_without_last[[col]]), 
                                       ifelse(is.na(last_known), default_value, last_known),
                                       df_without_last[[col]])
    }
  }
  
  # Recombine the processed data with the untouched last row
  df_filled <- rbind(df_without_last, last_row)
  
  return(df_filled)
}

workdata_backprop = backpropagate_NAs(workdata)
workdata = workdata_backprop %>% ungroup()
...
cat("Initial guess:\n", colnames(ini_guess_prevdata), "\n", ini_guess_v, ".\n If NAs are spotted (except year for now...), check the data. Missing series?")
...
eq_test <- function(x, eq_strs) {
  ...
  for (i in 1:ncol(exog_row)) {
    ...
    assign(col_name, var_value[[1]], envir = .GlobalEnv)
    # cat(col_name, "=", var_value[[1]], "\n")
  }

  eqs <- sapply(eq_strs, function(eq_str) {
    result <- tryCatch(
      {
        eval(parse(text = eq_str), envir = .GlobalEnv)
      },
      error = function(e) {
